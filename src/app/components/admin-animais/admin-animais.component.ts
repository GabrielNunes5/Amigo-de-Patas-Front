...
export class AdminAnimaisComponent {
  ...
  create = output<Partial<Animal>>();
  update = output<{ id: string; data: Partial<Animal> }>();
  ...
  animalForm = this.createAnimalForm();
  ...
  private createAnimalForm(): FormGroup {
    return this.fb.group({
      ...
      animalImageUrl: ['', Validators.required],
      animalAdopted: [false],
  ...
  handleSubmit(): void {
    ...
    if (selected) {
      this.update.emit({ id: selected.animalId, data });
    } else {
